...
public class GUI_INTERFACE_SETOR {

	public JFrame frame;
	int xMouse;
	int yMouse;
	String id;
	private JTextField textTmbhSaldo;
	/**
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					GUI_INTERFACE_SETOR window = new GUI_INTERFACE_SETOR();
	 ...
	 */
	public GUI_INTERFACE_SETOR() {
		initialize();
	}
	
	public GUI_INTERFACE_SETOR(String id) {
		this.id = id;
		initialize();
	}
	...
	public String setTambahSaldo(String saldo) {
		Map<String,Rekening> list = new HashMap<String,Rekening>();
		try {
			FileInputStream fis = new FileInputStream("database2");
			ObjectInputStream ois = new ObjectInputStream(fis);
		...
		return "Saldo gagal ditambahkan";
	}
	...
	private void initialize() {
		...
		JLabel lblJudul = new JLabel("Setor Duid");
		...
		JLabel lblTambahSaldo = new JLabel("Masukkan uang yang akan disetor:");
		lblTambahSaldo.setForeground(Color.WHITE);
		lblTambahSaldo.setFont(new Font("Candara Light", Font.PLAIN, 22));
		lblTambahSaldo.setBounds(32, 116, 326, 28);
		panel_1.add(lblTambahSaldo);
		
		textTmbhSaldo = new JTextField();
			...
			public void actionPerformed(ActionEvent e) {
				...
				if(Tsaldo.equals("") == true) {
					...
					if(Tsaldo.matches("[0-9]*$") == false) {
						...
						JOptionPane.showMessageDialog(frame,setTambahSaldo(Tsaldo));
